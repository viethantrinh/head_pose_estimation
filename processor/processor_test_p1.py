...
class ProcessorTest:
    """
    Processor for Head Pose Estimation (Testing)
    """
    ...
    def __init__(self, arg):
        ...
        # 5. Define the best MAE for test2
        self.best_mae_test_2 = float("inf")

        # 6. Define the best MAE for test1
        self.best_mae_test1 = float("inf")

        # 7. Best epoch tracking
        self.best_epoch_test1 = -1
        self.best_epoch_test_2 = -1

        # 8. Define the class weight
        self.class_weight = 1.0

        # 9. Define the regression weight
        self.reg_weight = 1.2

        # 10. Define the weight for yaw
        self.beta1 = 1.0

        # 11. Define the weight for pitch and roll
        self.beta2 = 0.2

        # 12. Define the Weight for yaw regression
        self.alpha1 = 1.0

        # 13. Define the weight for pitch and roll regression
        self.alpha2 = 0.7

        # 14. Define the stability for soft-argmax
    ...
    def eval(self, epoch=0, loader_name=[TEST_DATA_1, TEST_DATA_2]):
        ...
        for ln in loader_name:
            ...
            if ln == TEST_DATA_2 and mean_mae < self.best_mae_test_2:
                self.best_mae_test_2 = mean_mae
                self.best_epoch_test_2 = epoch
                self.print_log(
                    f'\tNew best result for {ln} with MAE: {self.best_mae_test_2:.4f}')

        # Print results summary
        self.print_log(
            "======================================================================================")
        self.print_log("TESTING RESULTS SUMMARY:")
        self.print_log(
            f'\tTest1 ({self.arg.test_dataset1}) MAE: {self.best_mae_test1:.4f}')
        self.print_log(
            f'\tTest2 ({self.arg.test_dataset2}) MAE: {self.best_mae_test_2:.4f}')
        self.print_log(
            "======================================================================================")
    ...
    def start(self):
        ...
        self.eval(epoch=0, loader_name=[TEST_DATA_1, TEST_DATA_2])

        best_overall = min(self.best_mae_test1, self.best_mae_test_2)
        self.print_log(
            f'Best overall MAE achieved: {best_overall:.4f} degrees')
