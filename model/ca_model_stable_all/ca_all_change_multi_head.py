...
class ConvolutionalMultiheadAttention(nn.Module):
    def __init__(self, in_channels, num_heads=8, kernel_size=3, padding=1):
        ...
        # For 144 channels with 4 heads, use a reasonable reduction
        self.qk_channels = max(in_channels // 4, num_heads * 4)  # Ensure at least 16 channels
    ...
    def _chunked_attention(self, attn_scores, chunk_size=32):
        """Compute attention in chunks to save memory for larger spatial dimensions"""
        B, num_heads, seq_len, _ = attn_scores.shape
        attn_weights = torch.zeros_like(attn_scores)
        
        for i in range(0, seq_len, chunk_size):
            end_i = min(i + chunk_size, seq_len)
            attn_weights[:, :, i:end_i, :] = F.softmax(attn_scores[:, :, i:end_i, :], dim=-1)
    ...
    def forward(self, x):
        ...
        # Compute attention scores with optional gradient checkpointing for memory efficiency
        # [B, num_heads, H*W, H*W]
        attn_scores = torch.matmul(q.transpose(-2, -1), k) * self.scale
        
        # Use more efficient attention computation for smaller spatial dimensions
        if H * W <= 64:  # 8x8 or smaller
            attn_weights = F.softmax(attn_scores, dim=-1)
        else:
            # For larger spatial dimensions, use chunked attention
            attn_weights = self._chunked_attention(attn_scores)
...
class CrossStreamFusion(nn.Module):
    def __init__(self, dim, num_heads=8):
        ...
        self.shared_cross_attention = BidirectionalCrossAttention(
            dim, num_heads)
    ...
    def forward(self, features):
        ...
        # For each stream as master, apply cross-attention with all other streams using shared attention module
        for i in range(4):
            master = features[i]
            cross_attended = []

            # Cross-attend with all other streams as slaves using shared attention module
            for j in range(4):
                if j != i:
                    # Apply shared bidirectional cross-attention between master and slave
                    attended = self.shared_cross_attention(master, features[j])
...
class Model(nn.Module):
    ...
    def forward(self, x_parts):
        ...
        for i in range(4):
            ...
            # Add positional embedding - make it adaptive to spatial dimensions
            ...
            print(x3.shape)
            # Get actual spatial dimensions from x3
            _, _, H, W = x3.shape
            pos_embed = pos_embed[:, :, None,
                                  None].expand(-1, -1, H, W)  # [1, 144, H, W]
