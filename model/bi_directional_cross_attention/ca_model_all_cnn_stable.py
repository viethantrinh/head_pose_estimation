...
class ConvolutionalCrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8, kernel_size=3, padding=1):
        super(ConvolutionalCrossAttention, self).__init__()
        ...
        # Use reduced channels for Q and K to optimize computation
        self.qk_channels = max(dim // 8, num_heads)
        self.qk_channels = (self.qk_channels // num_heads) * num_heads  # Ensure divisible by num_heads
        
        # Convolutional projections for Q, K, V
        self.query_conv = nn.Conv2d(dim, self.qk_channels, kernel_size=kernel_size, padding=padding)
        self.key_conv = nn.Conv2d(dim, self.qk_channels, kernel_size=kernel_size, padding=padding)
        self.value_conv = nn.Conv2d(dim, dim, kernel_size=kernel_size, padding=padding)
    ...
    def forward(self, query, key_value):
        ...
        q = self.query_conv(query)      # [B, qk_channels, H, W]
        k = self.key_conv(key_value)    # [B, qk_channels, H, W]
        v = self.value_conv(key_value)  # [B, C, H, W]
        
        # Reshape for multi-head attention
        qk_head_dim = self.qk_channels // self.num_heads
        q = q.view(batch_size, self.num_heads, qk_head_dim, H * W)  # [B, num_heads, qk_head_dim, H*W]
        k = k.view(batch_size, self.num_heads, qk_head_dim, H * W)  # [B, num_heads, qk_head_dim, H*W]
        v = v.view(batch_size, self.num_heads, self.head_dim, H * W)  # [B, num_heads, head_dim, H*W]
        
        # Compute attention scores
        attn_scores = torch.matmul(q.transpose(-2, -1), k) * self.scale  # [B, num_heads, H*W, H*W]
        attn_weights = F.softmax(attn_scores, dim=-1)
        attn_weights = self.dropout(attn_weights)
        
        # Apply attention to values
        out = torch.matmul(v, attn_weights.transpose(-2, -1))  # [B, num_heads, head_dim, H*W]
...
class BidirectionalCrossAttention(nn.Module):
    def __init__(self, dim, num_heads=8):
        ...
        self.cross_attn = ConvolutionalCrossAttention(dim, num_heads)
...
class CrossStreamFusion(nn.Module):
    def __init__(self, dim, num_heads=8):
        ...
        self.cross_attentions = nn.ModuleList([
            nn.ModuleList([BidirectionalCrossAttention(dim, num_heads)
...
class Model(nn.Module):
    ...
    def forward(self, x_parts):
        ...
        # 3. Apply multi-head self-attention for further refinement
