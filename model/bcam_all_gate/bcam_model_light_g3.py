...
class ConvolutionalMultiheadAttention(nn.Module):
    """
    T1: Thêm Gate SAU CONCAT trong Multi-Head Attention
    Đây chính là vị trí G1 mà senior đề xuất!
    """
    def __init__(self, in_channels, num_heads=8, kernel_size=3, padding=1):
        ...
        self.qk_channels = (self.qk_channels // num_heads) * num_heads
        ...
        # G1: Gate sau concat (trước out_conv)
    ...
    def forward(self, x):
        if torch.isnan(x).any() or torch.isinf(x).any():
            raise ValueError("ConvolutionalMultiheadAttention input contains NaN or Inf values.")
        ...
        q = q.view(batch_size, self.num_heads, self.qk_channels // self.num_heads, H * W)
        k = k.view(batch_size, self.num_heads, self.qk_channels // self.num_heads, H * W)
        ...
        # Reshape back to spatial dimensions (CONCAT operation)
        out = out.view(batch_size, C, H, W)

        # *** G1: GATE SAU CONCAT - ĐÂY LÀ VỊ TRÍ QUAN TRỌNG! ***
        # Tạo non-linearity và sparsity
        ...
        if torch.isnan(out).any() or torch.isinf(out).any():
            raise ValueError("ConvolutionalMultiheadAttention output contains NaN or Inf values.")
