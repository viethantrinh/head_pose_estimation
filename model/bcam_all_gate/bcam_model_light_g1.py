...
class ConvolutionalMultiheadAttention(nn.Module):
    def __init__(self, in_channels, num_heads=8, kernel_size=3, padding=1):
        ...
        # Remove internal gamma - will use external gate
    ...
    def forward(self, x):
        ...
        # Return without residual - will be handled externally with gate
        return out
...
class Model(nn.Module):
    def __init__(self):
        ...
        # ============ T1 GATING MECHANISM ============
        # g2: Stream-level gate (applied AFTER PE, BEFORE fusion)
        # Controls importance of each stream/patch - channel-wise selective filtering
        self.g2_stream_gate = nn.Sequential(
    ...
    def forward(self, x_parts):
        ...
        # 1. Go through 4 streams (multi-stream backbone)
        for i in range(4):
            ...
            # Apply g₂: Stream-level gate (T1 style - channel-wise filtering)
            # Learns which channels in which stream are more informative
            g2_gate = self.g2_stream_gate(x3)  # [B, 144, H, W]
            x3 = x3 * g2_gate  # Element-wise multiplication
