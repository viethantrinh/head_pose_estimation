...
class ConvolutionalMultiheadAttention(nn.Module):
    """
    G3: Gate sau concat trong Multi-Head Attention
    """
    def __init__(self, in_channels, num_heads=8, kernel_size=3, padding=1):
        ...
        # G3: Gate sau concat (trước out_conv)
...
class CrossStreamFusion(nn.Module):
    ...
    def forward(self, features):
        ...
        # For each stream as master, apply cross-attention with all other streams
        for i in range(4):
            master = features[i]
            cross_attended = []
...
class Model(nn.Module):
    """
    VERSION G1 + G2 + G3: Combines ALL three gates
    - G1: Stream-level gate (after PE, before fusion)
    - G2: Fusion-level gate (after fusion)
    - G3: MHA gate (after concat in multi-head attention)
    """
    def __init__(self):
