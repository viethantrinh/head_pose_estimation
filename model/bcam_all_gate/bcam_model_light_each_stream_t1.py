...
class CrossAttention(nn.Module):
    ...
    def forward(self, query, key_value):
        ...
        k = self.k_proj(key_value.flatten(2).transpose(1, 2))
        v = self.v_proj(key_value.flatten(2).transpose(1, 2))
        
        q = q.reshape(batch_size, H*W, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        ...
        x = torch.matmul(attn, v).transpose(1, 2).reshape(batch_size, H*W, C)
        x = self.out_proj(x)
...
class BidirectionalCrossAttention(nn.Module):
    """Gate after each stream's cross-attention"""
    def __init__(self, dim, num_heads=8):
    ...
    def forward(self, x1, x2):
        ...
        # Apply gating before residual
        gate_values = self.gate(x_fused)
        x_fused = x_fused * gate_values
...
class CrossStreamFusion(nn.Module):
    """
    T1A: Gate sau mỗi stream (4 gates độc lập)
    """
    def __init__(self, dim, num_heads=8):
        super(CrossStreamFusion, self).__init__()
        self.dim = dim

        # Shared cross-attention WITH GATE for each stream
    ...
    def forward(self, features):
        ...
        for i in range(4):
            ...
            for j in range(4):
                if j != i:
                    # Gate is applied inside this function
