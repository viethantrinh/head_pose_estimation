...
class ConvolutionalSelfAttention(nn.Module):
    def __init__(self, in_channels, kernel_size=3, padding=1):
        ...
        self.query_conv = nn.Conv2d(
            in_channels, in_channels // 8, kernel_size=kernel_size, padding=padding)
    ...
    def forward(self, x):
        if torch.isnan(x).any() or torch.isinf(x).any():
            raise ValueError(
                "ConvolutionalSelfAttention input contains NaN or Inf values.")
        ...
        if torch.isnan(out).any() or torch.isinf(out).any():
            raise ValueError(
                "ConvolutionalSelfAttention output contains NaN or Inf values.")
...
class EnhancedBiDirectionalCrossAttention(nn.Module):
    ...
    def forward(self, x_a, x_b):
        ...
        q_a = self.to_q_a(x_a_norm).view(batch_size, seq_len,
                                         self.num_heads, self.head_dim).transpose(1, 2)
        k_b = self.to_k_b(x_b_norm).view(batch_size, seq_len,
                                         self.num_heads, self.head_dim).transpose(1, 2)
        v_b = self.to_v_b(x_b_norm).view(batch_size, seq_len,
                                         self.num_heads, self.head_dim).transpose(1, 2)

        q_b = self.to_q_b(x_b_norm).view(batch_size, seq_len,
                                         self.num_heads, self.head_dim).transpose(1, 2)
        k_a = self.to_k_a(x_a_norm).view(batch_size, seq_len,
                                         self.num_heads, self.head_dim).transpose(1, 2)
        v_a = self.to_v_a(x_a_norm).view(batch_size, seq_len,
                                         self.num_heads, self.head_dim).transpose(1, 2)

        # Attention computation với temperature scaling
        # A → B attention
        attn_a_to_b = torch.matmul(
            q_a, k_b.transpose(-2, -1)) * self.scale / self.temperature.clamp(min=0.01)
        attn_a_to_b = F.softmax(attn_a_to_b, dim=-1)
        attn_a_to_b = F.dropout(attn_a_to_b, p=0.1, training=self.training)

        # B → A attention
        attn_b_to_a = torch.matmul(
            q_b, k_a.transpose(-2, -1)) * self.scale / self.temperature.clamp(min=0.01)
        attn_b_to_a = F.softmax(attn_b_to_a, dim=-1)
        attn_b_to_a = F.dropout(attn_b_to_a, p=0.1, training=self.training)

        # Apply attention
        x_a_attends_to_b = torch.matmul(attn_a_to_b, v_b).transpose(
            1, 2).contiguous().view(batch_size, seq_len, -1)
        x_b_attends_to_a = torch.matmul(attn_b_to_a, v_a).transpose(
            1, 2).contiguous().view(batch_size, seq_len, -1)

        # Cross-modal fusion
        cross_attended = self.cross_fusion(
...
class SpatialRelationModule(nn.Module):
    ...
    def __init__(self, dim):
        ...
        # We'll create position embeddings dynamically based on input size
        self.relative_pos_embed = None

    def _create_position_embeddings(self, seq_len, device):
        """Create position embeddings for each patch dynamically"""
        if self.relative_pos_embed is None or self.relative_pos_embed[0].size(1) != seq_len:
            self.relative_pos_embed = nn.ParameterList([
                nn.Parameter(torch.randn(
                    1, seq_len, self.dim, device=device) * 0.02)
                for _ in range(4)
            ])
            # Move to correct device
            for i in range(4):
                self.relative_pos_embed[i] = self.relative_pos_embed[i].to(
                    device)

    def forward(self, features, patch_indices):
        """
        features: List of 4 patch features [B, L, D] where L = height * width
        ...
        batch_size, seq_len, dim = features[0].shape
        device = features[0].device

        # Create position embeddings if needed
        self._create_position_embeddings(seq_len, device)

        enhanced_features = []

        for i, feat in enumerate(features):
            # Get position encoding for this patch
            pos_encoding = self.relative_pos_embed[i].expand(
                batch_size, -1, -1)
...
class MultiScaleFusionModule(nn.Module):
    ...
    def __init__(self, dim, scales=[1, 3, 5]):
        super(MultiScaleFusionModule, self).__init__()
        self.scales = scales
        self.dim = dim

        # Multi-scale convolutions with proper padding to preserve sequence length
        self.scale_convs = nn.ModuleList()
        for scale in scales:
            # Calculate padding to maintain sequence length: padding = (kernel_size - 1) // 2
            padding = (scale - 1) // 2
            conv = nn.Sequential(
                nn.Conv1d(dim, dim, kernel_size=scale,
                          padding=padding, groups=max(1, dim//8)),
                nn.BatchNorm1d(dim),
                nn.GELU()
            )
    ...
    def forward(self, x):
        ...
        for conv in self.scale_convs:
            scale_out = conv(x_conv)  # [B, D, L]

            # Ensure output length matches input length
            if scale_out.size(2) != seq_len:
                scale_out = F.interpolate(
                    scale_out, size=seq_len, mode='linear', align_corners=False)

            scale_out = scale_out.transpose(1, 2)  # Back to [B, L, D]
...
class AdaptiveNeighborhoodFusionTransformer(nn.Module):
    ...
    def forward(self, features):
        ...
        # Self-attention for final refinement
        global_fused_attn, _ = self.self_attn(
            global_fused, global_fused, global_fused)
...
class Model(nn.Module):
    def __init__(self):
        ...
        for i in range(4):
            ...
            self.patch_pos_embed.append(nn.Parameter(
                pos + learnable_offset, requires_grad=True))
    ...
    def forward(self, x_parts):
        ...
        for i in range(4):
            ...
            pos_embed = pos_embed[:, :, None,
                                  None].expand(-1, -1, x3.size(2), x3.size(3))
