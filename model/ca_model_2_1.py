...
class BidirectionalCrossAttention(nn.Module):
    """Bidirectional cross-attention between two feature maps"""
    def __init__(self, dim, num_heads=8):
        super(BidirectionalCrossAttention, self).__init__()
        self.cross_attn_a_to_b = CrossAttention(dim, num_heads)
        self.cross_attn_b_to_a = CrossAttention(dim, num_heads)
    ...
    def forward(self, x_a, x_b):
        """
        Bidirectional cross-attention between two feature maps as per the paper:
        x_q_x = CrossAttn(Q_x, K_objs, V_objs)
        x_q_objs = CrossAttn(Q_objs, K_x, V_x)
        x = x + tanh(γ) * Linear(Concat(x_q_x, x_q_objs))
        
        Args:
            x_a: Features from stream A [B, C, H, W]
            x_b: Features from stream B [B, C, H, W]
            
        Returns:
            Enhanced features for stream A [B, C, H, W]
        """
        # A->B direction: A attends to B
        x_a_to_b = self.cross_attn_a_to_b(x_a, x_b)
        
        # B->A direction: B attends to A
        x_b_to_a = self.cross_attn_b_to_a(x_b, x_a)
        
        # Concatenate and fuse the cross-attention results
        x_cat = torch.cat([x_a_to_b, x_b_to_a], dim=1)
        x_fused = self.fusion(x_cat)
        
        # Apply learnable scaling parameter and residual connection
        x_enhanced = x_a + torch.tanh(self.gamma) * x_fused
        
        return x_enhanced

class CrossStreamFusion(nn.Module):
    """Fuse 4 streams using bidirectional cross-attention"""
    def __init__(self, dim, num_heads=8):
        super(CrossStreamFusion, self).__init__()
        self.dim = dim
        
        # For each master stream, create cross-attention modules to all other streams
        # This fixed structure correctly creates cross-attention modules for all master-slave pairs
        self.cross_attentions = nn.ModuleList()
        
        for i in range(4):  # For each master stream
            slave_attns = nn.ModuleList()
            for j in range(4):  # For each potential slave stream
                if j != i:  # Skip self-attention
                    slave_attns.append(BidirectionalCrossAttention(dim, num_heads))
            self.cross_attentions.append(slave_attns)
        ...
        # Add a feed-forward network with scaling factor as mentioned in the paper
    ...
    def forward(self, features):
        ...
        for i in range(4):
            ...
            # Average the cross-attended features (master enhanced by all slaves)
        ...
        # Apply feed-forward network with scaling as per the paper:
        # x = x + tanh(γ_d) * FeedForward(x)
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Define the self attention part for final refinement
