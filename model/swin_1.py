...
class SwinBackbone(nn.Module):
    r""" Swin Transformer Backbone
        Replaces the BaseModel to maintain the same interface
    ...
    def __init__(self, img_size=64, patch_size=2, in_chans=3, embed_dim=32,
        ...
        # Add conversion layers to match original BaseModel output channels
        self.norm1 = norm_layer(embed_dim)
        self.norm2 = norm_layer(embed_dim * 2)
        self.norm3 = norm_layer(embed_dim * 4)
        
        # Convert to match original output channels (32, 128, 144)
        self.conv_adapt1 = nn.Conv2d(embed_dim, 32, kernel_size=1)
        self.conv_adapt2 = nn.Conv2d(embed_dim * 2, 128, kernel_size=1)
        self.conv_adapt3 = nn.Conv2d(embed_dim * 4, 144, kernel_size=1)
        
        # Pooling to match spatial dimensions
    ...
    def forward(self, x):
        ...
        # Store intermediate features to match BaseModel interface
        features = []
        
        for i, layer in enumerate(self.layers):
            x = layer(x)
            
            # Convert back to spatial format for each stage
            if i == 0:  # After first stage (embed_dim channels)
                B, L, C = x.shape
                H = W = int(np.sqrt(L))
                x_spatial = x.view(B, H, W, C).permute(0, 3, 1, 2)  # B, C, H, W
                x_spatial = self.norm1(x.view(B, L, C)).view(B, H, W, C).permute(0, 3, 1, 2)
                x1 = self.conv_adapt1(x_spatial)  # Convert to 32 channels
                x1 = self.dropout(x1)
                x1_pool = self.pool(x1)  # Pool for next stage
                features.append(x1)
                
            elif i == 1:  # After second stage (embed_dim * 2 channels)
                B, L, C = x.shape
                H = W = int(np.sqrt(L))
                x_spatial = x.view(B, H, W, C).permute(0, 3, 1, 2)
                x_spatial = self.norm2(x.view(B, L, C)).view(B, H, W, C).permute(0, 3, 1, 2)
                x2 = self.conv_adapt2(x_spatial)  # Convert to 128 channels
                x2 = self.dropout(x2)
                x2_pool = self.pool(x2)  # Pool for next stage
                features.append(x2)
                
            elif i == 2:  # After third stage (embed_dim * 4 channels)
                B, L, C = x.shape
                H = W = int(np.sqrt(L))
                x_spatial = x.view(B, H, W, C).permute(0, 3, 1, 2)
                x_spatial = self.norm3(x.view(B, L, C)).view(B, H, W, C).permute(0, 3, 1, 2)
                x3 = self.conv_adapt3(x_spatial)  # Convert to 144 channels
                x3 = self.dropout(x3)
                features.append(x3)

        return features[0], features[1], features[2]  # Return x1, x2, x3 like BaseModel


# Keep your existing attention modules
...
class Model(nn.Module):
    def __init__(self):
        ...
        # Replace BaseModel with SwinBackbone
        self.base_model = SwinBackbone(
            img_size=64,           # Adjust based on your input size
            patch_size=2,          # Smaller patch size for better fine-grained features
            in_chans=3,
            embed_dim=32,          # Start with 32 to match your original first stage
            depths=[2, 2, 2],      # 3 stages like your original model
            num_heads=[2, 4, 8],   # Increasing attention heads
            window_size=4,         # Smaller window for 64x64 images
            mlp_ratio=4.,
            qkv_bias=True,
            drop_rate=0.1,
            attn_drop_rate=0.1,
        ...
        # Keep the rest of your model unchanged
        ...
        # Position embeddings remain the same
        ...
        # Classification heads remain the same
    ...
    def forward(self, x_parts):
        ...
        # Process 4 streams with Swin backbone
        for i in range(4):
            _, _, x3 = self.base_model(x_parts[i])  # Swin backbone outputs
            ...
            pos_embed = pos_embed[:, :, None, None].expand(-1, -1, x3.shape[2], x3.shape[3])
            
            x3 = x3 + pos_embed
            x_features.append(x3)
            
        # Fusion and classification remain the same
        x4 = torch.cat(x_features, dim=1)
